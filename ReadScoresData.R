...
head(results)
results[order(char_min)[1:5],]
match(char_min, teams_scores)
...
name_search("Michigan", teams_bartorvik1)
#Read back in sorted teams
MTeams_sorted <- read.csv("bartorvik_sort.csv", header = T, sep = ",")
head(MTeams_sorted)
New <- MTeams_sorted[MTeams_sorted$X == 1, 1:5]
MTeams <- rbind(MTeams_keep, New)
Unmatched <- MTeams_sorted[MTeams_sorted$X == 0, 1:5]
bartorvik2 <- teams_bartorvik1[!(teams_bartorvik1 %in% MTeams$bartorvik)]
#We save the data from the 42 teams that are unmatched
